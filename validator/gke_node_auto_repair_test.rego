...
package templates.gcp.GCPGKENodeAutoRepairConstraintV1

import data.validator.gcp.lib as lib

all_violations[violation] {
	resource := data.test.fixtures.gke_node_auto_repair.assets[_]
	constraint := data.test.fixtures.gke_node_auto_repair.constraints.enable_auto_repair
...
test_enable_auto_repair_set_to_true {
...
test_enable_auto_repair_set_to_false {
	violation := all_violations[_]
...
test_multiple_node_pools {
	# Autorepair is set to true on one of them and false on the other one.
	violation := all_violations[_]
